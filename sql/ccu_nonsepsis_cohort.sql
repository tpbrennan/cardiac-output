...
with population as (
  ...
  select distinct id.subject_id, id.icustay_id, id.hadm_id, 
      id.icustay_intime,
...
--select count(icustay_id) from population;--6,877 rows
-- final cohort, exclude pacemakers
, final_cohort as (
  select p.* from population p 
    where not exists (select * from tbrennan.pacemaker_cohort pc where p.subject_id = pc.subject_id and p.hadm_id = pc.hadm_id)
      and not exists (select * from tbrennan.angus_sepsis_cohort sc where p.subject_id = sc.subject_id and p.hadm_id = sc.hadm_id)
)
--select count(distinct icustay_id) from final_cohort; -- 4,444 icustay_id
...
select * from assemble; --3,796 rows
