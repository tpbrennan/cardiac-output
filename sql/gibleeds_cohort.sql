...
with population as (
  ...
  select distinct id.subject_id, id.icustay_id, id.hadm_id, 
    ...
    join mimic2v26.icd9 icd on id.subject_id = icd.subject_id and id.hadm_id = icd.hadm_id
      where id.icustay_admit_age < 90
        and icd.code like '%578%'
        and id.icustay_id is not null
        and id.gender is not null
        and id.icustay_age_group = 'adult'
)
--select count(distinct icustay_id) from population;--1,334 rows
...
--select count(distinct icustay_id) from final_cohort; -- 1,220 icustay_id
